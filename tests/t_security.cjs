...
(async () => {
  ...
  const pwned = await p.evaluate(async (id) => {
    const i = await fetch('/api/invoices/' + id, { headers: { 'X-Requested-With': 'hk' } }).then(r => r.json());
    // force the raw payload through the print path as if it had been stored
    i.payments = [{ date: i.date, mode: '<iframe srcdoc="&lt;script&gt;window.__PWNED=1&lt;/script&gt;"></iframe>', amount: 1, ref: 'x' }];
    document.getElementById('printarea').innerHTML = billHTML(i);
    await new Promise(r => setTimeout(r, 600));
    return { pwned: !!window.__PWNED, iframes: document.querySelectorAll('#printarea iframe').length };
  }, inv.data.id);
  ok('print template cannot execute an injected payload', !pwned.pwned && pwned.iframes === 0, JSON.stringify(pwned));

  // =====  FINDING 11: inline onclick handlers dead under CSP  =====
  await p.evaluate(() => location.hash = 'invoices'); await p.waitForTimeout(1600);
  const btn = await p.$('#ilist button[data-do="open"]');
  ...
  const closeBtn = await p.$('.mask [data-do="close"], .mask .x');
